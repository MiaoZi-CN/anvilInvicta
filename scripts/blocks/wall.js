...
const electromagneticWall = extend(Wall, "electromagnetic-wall", {
 ...
 setStats() {
  ...
  this.stats.add(Stat("maxChanceDeflect"), this.chanceDeflect + this.addChanceDeflect);//显示总护甲和总反弹概率
...
electromagneticWall.buildType = () =>
 extend(Wall.WallBuild, electromagneticWall, {
  ...
  updateTile() {
   this.super$updateTile();
   if (this.canConsume()) {
    this.armor = 28;//有电力供应时额外的护甲值
    this.chanceDeflect = 30;//有电力供应时额外的概率
   } else {
    this.armor = this.block.armor;//没有电力供应时恢复基础护甲值
    this.chanceDeflect = this.block.chanceDeflect;//没有电力供应时恢复基础概率
   }
  },
  draw() {
   // this.super$draw();
