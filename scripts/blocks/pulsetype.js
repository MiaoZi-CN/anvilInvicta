...
drill.buildType = () => {
 ...
 return extend(DrillBuild, block, {
  ...
  buildConfiguration(table) {
   // 添加空检查
   if (drillTileMap == null || drillTileMap.size == 0) {
    table.add("No ore found in range");
    return;
   }
  ...
  draw() {
   ...
   // 修复：避免 dominantItem 为 null 时调用 get()
   if (this.dominantItem == null || drillTileMap == null) return;
   ...
   seq.each(tile => {
    ...
    Draw.z(Layer.block);
    Lines.stroke(2);
    Lines.poly(tx, ty, 4, 8, Time.time * 0.5);
   });
  },
  ...
  setDrillItem(item) {
   ...
   if (drillTileMap != null && item != null) {
